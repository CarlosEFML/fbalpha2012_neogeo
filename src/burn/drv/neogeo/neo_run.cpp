...
static INT32 InitCache(void)
{
	...
	if(BurnUseCache)
	{
		...
		if (!BurnCacheFile)
		{
			printf("Error opening %s! Convert the game with romcnv.", CacheName);
			sleep(5);
			exit(0);
		}
		...
		for(int i = 0; i < step; i++)
		{
			...
			Progress = (float)i / CacheSize  * 5.0;
			snprintf(txt, sizeof(txt), "Loading crom in RAM : %d/%d MB", i, step);
			ProgressBar(Progress - BarOffset, txt);
		}
		snprintf(txt, sizeof(txt), "Loading crom in RAM : %d/%d MB", step, step);
		ProgressBar(Progress - BarOffset, txt);

		CacheRead += step;
		step = (SpriteVmSize)/(1*MB);

		for(int i = 0; i < step; i++)
		{
			bytesRead = fread(NeoSpriteROM_WIIVM + (i*MB), 1*MB, 1, BurnCacheFile);
			Progress = ((float)i + CacheRead) / CacheSize  * 5.0;
			snprintf(txt, sizeof(txt), "Loading crom in VM : %d/%d MB", i, step);
			ProgressBar(Progress - BarOffset, txt);
		}
		snprintf(txt, sizeof(txt), "Loading crom in VM : %d/%d MB", step, step);
		ProgressBar(Progress - BarOffset, txt);
		...
		printf("\33[2K\r");
		printf("%9s %s\r",  "", "Loading srom file");
...
static INT32 ReadSoundCache(UINT8* Audio)
{
	...
	BurnCacheFile = fopen(CacheName, "rb");
	if (!BurnCacheFile) printf("Error opening %s!\r", CacheName);

	size_t bytesRead = 0;
	UINT32 step = nYM2610ADPCMASize[nNeoActiveSlot]/(1*MB);
	float Progress;
	float BarOffset = 1.0;

	for(int i = 0; i < step; i++)
	{
		bytesRead = fread(Audio + (i*MB), 1*MB, 1, BurnCacheFile);
		Progress = ((float)i + CacheRead) / CacheSize  * 5.0;
		snprintf(txt, sizeof(txt), "Loading vrom : %d/%d MB", i, step);
		ProgressBar(Progress - BarOffset, txt);
	}

	snprintf(txt, sizeof(txt), "Loading vrom : %d/%d MB", step, step);
	ProgressBar(Progress - BarOffset, txt);
	fclose(BurnCacheFile);
	ProgressBar(4.0, txt);
}
...
#ifdef WII_VM
			printf("Loading Neo68KROM failed. Not enough memory! exiting...\r");
			sleep(5);
			exit(0);
#endif
		return 1;
...
#ifdef WII_VM
			printf("Loading NeoZ80ROM failed. Not enough memory! exiting...\r");
      sleep(5);
      exit(0);
#endif
		return 1;
...
#ifdef WII_VM
         printf("Loading YM2610ADPCMAROM failed. Not enough memory! exiting...\r");
         sleep(5);
         exit(0);
#endif
			return 1;
...
#ifdef WII_VM
         printf("Loading YM2610ADPCMBROM failed. Not enough memory! exiting...\r");
         sleep(5);
         exit(0);
#endif
			return 1;
...
//AKI
// static inline void neogeoSynchroniseZ80_IMPL(INT32 nExtraCycles)
...
//AKI
#include <3ds.h>
Thread sndThreadHandle;
Handle sndThreadEventStart;
int sndThreadStarted = 0;
volatile INT32 cycles = 0;

// void sndThread(void *args) {
//   while (1) {
//       svcWaitSynchronization(sndThreadEventStart, U64_MAX);
//       svcClearEvent(sndThreadEventStart);
//       neogeoSynchroniseZ80_IMPL(cycles);
//       // BurnYM2610Update(pBurnSoundOut, nBurnSoundLen);
//   }
// }
//
// void BurnYM2610UpdateThreaded() {
//   // svcSignalEvent(sndThreadEventStart);
//   BurnYM2610Update(pBurnSoundOut, nBurnSoundLen);
// }
//
// static inline void neogeoSynchroniseZ80(INT32 nExtraCycles)
...
void initSndThreaded() {
  // svcCreateEvent(&sndThreadEventStart, RESET_ONESHOT);
  // APT_SetAppCpuTimeLimit(80);
  // sndThreadHandle = threadCreate(sndThread, 0, STACKSIZE, 0x18, 1, true);
}
	...
	if (nNeoSystemType & NEO_SYS_CART) {
		...
		if (((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) != HARDWARE_SNK_DEDICATED_PCB) && (BurnDrvGetHardwareCode() & HARDWARE_SNK_CONTROLMASK) != HARDWARE_SNK_TRACKBALL) {
			switch (NeoSystem & 0x1f) {
				...
				case 0x0e: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 4.0 BIOS\n")); break; }
				case 0x0f: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 3.3 BIOS\n")); break; }
				case 0x10: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 3.2 BIOS\n")); break; }
				case 0x11: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 3.1 BIOS\n")); break; }
				case 0x12: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 3.0 BIOS\n")); break; }
				case 0x13: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 2.3 BIOS\n")); break; }
				case 0x14: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 2.3 (alt) BIOS\n")); break; }
				case 0x15: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 2.2 BIOS\n")); break; }
				case 0x16: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 2.1 BIOS\n")); break; }
				case 0x17: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 2.0 BIOS\n")); break; }
				case 0x18: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 1.3 BIOS\n")); break; }
				case 0x19: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 1.2 BIOS\n")); break; }
				case 0x1a: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 1.2 (alt) BIOS\n")); break; }
				case 0x1b: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 1.1 BIOS\n")); break; }
				case 0x1c: { bprintf(PRINT_IMPORTANT, _T("Emulating using Universe BIOS ver. 1.0 BIOS\n")); break; }
				case 0x1d: { bprintf(PRINT_IMPORTANT, _T("Emulating using NeoOpen BIOS v0.1 beta BIOS\n")); break; }
			}
		}
...
//		memset(NeoCallback, 0, sizeof(NeoCallback));
	...
	if (nNeoSystemType & NEO_SYS_PCB) {
		BurnLoadRom(Neo68KBIOS, 0x00080 +     31, 1);
	}

	if ((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_SNK_MVS) {
		BurnLoadRom(NeoZ80BIOS,		0x00000 + 32, 1);
		BurnLoadRom(NeoTextROMBIOS,	0x00000 + 33, 1);
		BurnLoadRom(NeoZoomROM,		0x00000 + 34, 1);
	} else {
		...
		if (nNeoSystemType & NEO_SYS_PCB) {
			bZ80BIOS = false;
			BurnLoadRom(NeoTextROMBIOS,	0x00080 + 33, 1);
			BurnLoadRom(NeoZoomROM,		0x00080 + 34, 1);
		} else {
			BurnLoadRom(NeoZ80BIOS,		0x00080 + 32, 1);
			BurnLoadRom(NeoTextROMBIOS,	0x00080 + 33, 1);
			BurnLoadRom(NeoZoomROM,		0x00080 + 34, 1);
		}
	}
